...
const ckeck_me_category = (id_ca: number): string => {
  ...
  list_category.value.forEach((element) => {
    if (element.id == id_ca) cat_name = element.label;
  });
  return cat_name;
};
...
const onUpdateAddAlbum = async () => {
  if (id_choose.value == 1) {
    let res = await update_album({
      ...
      .then((res) =>
        toast_function(
          "Successfully update album #" + chosen_album.value?.id + "",
          "success"
        )
      )
      .catch((error) =>
        toast_function(
          "Failed to update " + chosen_album.value?.id + "",
          "error"
        )
      );
  } else {
    ...
    let res = await create_album({
      ...
      .then((res) => toast_function("Successfully new album added", "success"))
      .catch((error) =>
...
<template>
  <div class="container-fluid pt-4">
    <button class="btn btn-outline-warning" @click="onCLickAdd">
      Add album
    </button>
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              ...
              >
                ...
                <option
                  v-for="option_cat in list_category"
                  :value="option_cat.id"
                >
                  {{ option_cat.label }}
                </option>
